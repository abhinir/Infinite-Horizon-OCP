function [x_nom, u_nom, cost, K, vk, Sk] = ILQR(model, x0, xg, u_nom, horizon,...
                    Q, R, QT, maxIte)
...
%             end
    ...
    state_err = compute_state_error(x_nom(:,end), model.Xg, model.name);
    ...
    for i=horizon:-1:1
        ...
        state_err = compute_state_error(x_nom(:,i), model.Xg, model.name);
%         vk(:,i) = Sk(:,:,i)*state_err + Sk(:,:,i)*x_nom(:,i);
    ...
    if ((abs(cost_change) < 0.01) || iter == maxIte || p<1e-4)
        criteria = false;
        %disp('converged');
