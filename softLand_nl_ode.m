...
torque = [u(1);u(2);u(3)]*100;
F = [u(4);u(5);u(6)]*1000;
omega = [w1;w2;w3];

out(1:3) = (1/cos(theta))*M*omega;
out(4:6) = -inv(model.J)*cross(omega,model.J*omega) + inv(model.J)*torque;
out(7:9) = [v1*1000;v2*1000;v3*1000]/1000;
out(10)  = F(1)/(m*1000);
out(11)  = F(2)/(m*1000);
% if r3> 0 && r3<1e-5
%     out(12) = F(3)/(m*1000);
% else
%     out(12)  = F(3)/(m*1000) - 3.7114/1000;
% end
out(12)  = F(3)/(m*1000) - 3.7114/10000;
out(13)  = -sqrt(F'*F)/(225*3.7114);
% out(12)  = F(3)/(m*1000) -(3.7114-3.7114*(tanh(10000*-r3)))/2e3;
